...
set.seed(12345)
labeledsamples=read.delim("k:/finalresults/nuclear/nuclearselfBLM/subdpfeatures.txt",header = T)
sortfeatures=read.delim("k:/finalresults/nuclear/nuclearselfBLM/sortfeaturesASC.txt",header =F)
...
for (i in 1:10) {
  ...
  dim(testset)
  dim(Nexttrainset)
  rf.model <- randomForest(dplabel ~ ., data = Nexttrainset)
  rf.pred <- predict(rf.model, testset[,-ncol(testset)])
  confusionmat<-table(pred = rf.pred, true = as.factor(testset[,ncol(testset)]))
...
write.table(dfresults,file ="k:/finalresults/nuclear/nuclearselfBLM/AUC_AUPR/ten_fold_newprogram/dfresults_RF.txt",quote = F,sep = "\t",row.names = T,col.names = T)
